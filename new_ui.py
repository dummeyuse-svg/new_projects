...
def analyze_uplift(current_profile, baseline_profile):
    ...
    raw_diff  = baseline_profile.astype(np.float32) - current_profile.astype(np.float32)
    detrended = robust_detrend(raw_diff,
                               order           = CONFIG["DETREND_ORDER"],
                               inlier_fraction = CONFIG["DETREND_INLIER_FRACTION"])
...
class TemporalProfileSmoother:
    ...
    def get_smoothed(self):
        if not self._buffer:
            return None
        return np.median(np.stack(list(self._buffer), axis=0), axis=0).astype(np.float32)
...
# ══════════════════════════════════════════════════════════════════
# CAMERA SELECTION DIALOG  ← NEW
# ══════════════════════════════════════════════════════════════════
class CameraSelectDialog(tk.Toplevel):
    """
    Shown once at startup.  Sets self.result to one of:
        'picamera2'  – use Raspberry Pi camera via Picamera2
        'webcam'     – use USB/laptop webcam via OpenCV
        None         – user closed the window (exit)
    """
    def __init__(self, parent):
        super().__init__(parent)
        self.title("Select Camera Backend")
        self.configure(bg=PANEL_BG)
        self.resizable(False, False)
        self.grab_set()
        self.result = None
        ...
        # ── header ────────────────────────────────────────────────
        tk.Label(self,
                 text="PCB WARPAGE INSPECTOR",
                 font=("Segoe UI", 14, "bold"),
                 bg=PANEL_BG, fg=ACCENT, pady=6
                 ).pack(padx=30, pady=(20, 4))

        tk.Label(self,
                 text="Select the camera source to use for this session:",
                 font=("Segoe UI", 10),
                 bg=PANEL_BG, fg=MUTED
                 ).pack(padx=30, pady=(0, 16))

        # ── option cards ──────────────────────────────────────────
        cards = tk.Frame(self, bg=PANEL_BG)
        cards.pack(padx=30, pady=(0, 10))

        self._make_card(
            cards,
            icon   = "📷",
            title  = "Picamera2",
            desc   = "Raspberry Pi camera module\n(IMX219 / HQ / GS etc.)",
            value  = "picamera2",
            col    = 0,
            avail  = PICAMERA_AVAILABLE,
        )

        self._make_card(
            cards,
            icon   = "🖥",
            title  = "Webcam / USB Camera",
            desc   = "Standard webcam or USB camera\nvia OpenCV VideoCapture",
            value  = "webcam",
            col    = 1,
            avail  = True,   # always try; fail gracefully at runtime
        )

        # ── webcam index selector ─────────────────────────────────
        idx_frame = tk.Frame(self, bg=PANEL_BG)
        idx_frame.pack(padx=30, pady=(4, 0))
        tk.Label(idx_frame, text="Webcam index:",
                 font=("Segoe UI", 9), bg=PANEL_BG, fg=MUTED
                 ).pack(side="left")
        self._idx_var = tk.StringVar(value=str(CONFIG["WEBCAM_INDEX"]))
        tk.Entry(idx_frame, textvariable=self._idx_var, width=4,
                 bg=BTN_BG, fg=TEXT_FG, insertbackground=ACCENT,
                 relief="flat", font=FONT_MONO
                 ).pack(side="left", padx=(6, 0))
        tk.Label(idx_frame,
                 text="(only used for Webcam mode)",
                 font=("Segoe UI", 8), bg=PANEL_BG, fg=MUTED
                 ).pack(side="left", padx=(8, 0))

        tk.Frame(self, bg=BORDER, height=1).pack(fill="x", padx=20, pady=14)
    ...
    # ── card builder ──────────────────────────────────────────────
    def _make_card(self, parent, icon, title, desc, value, col, avail):
        border_color = BORDER if avail else "#2a2a2a"
        fg_color     = TEXT_FG if avail else MUTED

        frame = tk.Frame(parent, bg="#1c2128",
        ...
        tk.Label(frame, text=icon, font=("Segoe UI", 28),
                 bg="#1c2128", fg=fg_color).pack()
        tk.Label(frame, text=title,
                 font=("Segoe UI", 11, "bold"),
                 bg="#1c2128", fg=fg_color).pack(pady=(4, 2))
        tk.Label(frame, text=desc,
                 font=("Segoe UI", 8), bg="#1c2128", fg=MUTED,
                 justify="center").pack()

        if not avail:
            tk.Label(frame, text="⚠ Not installed",
                     font=("Segoe UI", 8), bg="#1c2128", fg=ORANGE
                     ).pack(pady=(4, 0))
            return   # no button if unavailable

        btn = tk.Button(
            frame,
            text   = f"Use {title}",
            command= lambda v=value: self._select(v),
            bg     = ACCENT if value == "picamera2" else BTN_BG,
            fg     = DARK_BG if value == "picamera2" else TEXT_FG,
    ...
    def _select(self, value):
        # Save webcam index before closing
        try:
            CONFIG["WEBCAM_INDEX"] = int(self._idx_var.get())
        except ValueError:
            pass
        self.result = value
        self.destroy()
...
# ══════════════════════════════════════════════════════════════════
# CAMERA BACKENDS  ← unified interface
# ══════════════════════════════════════════════════════════════════
class PiCamera2Backend:
    """Wraps Picamera2 — capture_array() returns RGB numpy array."""

    def __init__(self):
        self._cam = Picamera2()
        cfg = self._cam.create_still_configuration(
            main={
                "size":   CONFIG["CAPTURE_RESOLUTION"],
                "format": "RGB888",
            },
            controls={
                "AfMode":       0,
        ...
        self._cam.start()
        time.sleep(1.5)   # let AEC settle

    def capture_array(self):
        """Returns RGB uint8 array."""
        return self._cam.capture_array("main")   # already RGB888
...
class WebcamBackend:
    """Wraps cv2.VideoCapture — capture_array() returns RGB numpy array."""
...
class PCBApp(tk.Tk):
    ...
    def __init__(self, camera_backend: str):
        super().__init__()
        self.camera_backend = camera_backend   # 'picamera2' or 'webcam'
        self.title(f"PCB Warpage Detection System  [{camera_backend}]")
        self.configure(bg=DARK_BG)
        self.resizable(True, True)
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr,
                 text=f"⬡  PCB WARPAGE INSPECTOR  [{self.camera_backend}]",
                 font=("Segoe UI", 13, "bold"),
                 bg=PANEL_BG, fg=ACCENT, padx=16, pady=10
                 ).grid(row=0, column=0, sticky="w")
    ...
    # ── CAMERA START  (backend-aware) ─────────────────────────────
    def _start_camera(self):
        try:
            if self.camera_backend == "picamera2":
                self.cam = PiCamera2Backend()
            else:
                self.cam = WebcamBackend(CONFIG["WEBCAM_INDEX"])

            self._preview_running = True
            self._preview_thread  = threading.Thread(
                target=self._preview_loop, daemon=True)
            self._preview_thread.start()
            self._log(f"Camera started [{self.camera_backend}]. Live preview active.")
            self._set_status(f"Camera live [{self.camera_backend}]")

        except Exception as e:
            self._log(f"⚠  Camera error: {e}")
            self._set_status("No camera — file mode only")
            self.cam = None

    # ── PREVIEW LOOP  (identical for both backends) ───────────────
    def _preview_loop(self):
        while self._preview_running:
            try:
                # Both backends return RGB; convert once to BGR for OpenCV
                rgb  = self.cam.capture_array()
                bgr  = cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR)
                bgr  = normalize_image(bgr)
                gray = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY)
                ...
                pass
            # Picamera2 is slower to respond; give it a little more time
            time.sleep(0.06 if self.camera_backend == "picamera2" else 0.04)
...
# ══════════════════════════════════════════════════════════════════
if __name__ == "__main__":
    # Invisible root needed so the dialog has a parent
    root = tk.Tk()
    root.withdraw()

    dialog = CameraSelectDialog(root)
    ...
    app = PCBApp(camera_backend=chosen)
    app.mainloop()
